...
import type { AnyStep } from "@/lib/types";
...
export function StepItem({ step, traceStart, index, depth = 0 }: StepItemProps) {
  ...
  return (
    <div className={`${depth > 0 ? "ml-4" : ""}`}>
      ...
      <div
        className={`
          flex items-start gap-2.5 py-2 px-3 rounded-lg cursor-pointer
          hover:bg-gray-50 transition-colors group
          ${isLoop ? "cursor-default" : ""}
        `}
      ...
      {/* Step detail (non-loop) */}
      {!isLoop && open && <StepDetail step={step} />}

      {/* Loop iterations */}
      {isLoop && loopsOpen && (
        <div className="ml-8 mt-1 mb-2">
          {(step as import("@/lib/types").LoopStep).iterations.map((iter) => (
